...
// max 15 chars
export const shortname: string = 'satisfactoryja'

// 固定ポストのURI
// 取得方法は、SkyFeedで該当ポストを開き、Copy JSONで得られるJSON内のuriをコピーする
// 画面ではこの配列の逆順に表示される(一番下のものが一番上に表示される)
const fixedPostUris: string[] = [
	'at://did:plc:wdoyybvxbazpbpxpvyntlzsq/app.bsky.feed.post/3kl7wpn6epp2u'
]
...
export const handler = async (ctx: AppContext, params: QueryParams) => {
	...
	let builder = ctx.db
		...
		.selectAll()
		.where('lang1', '=', 'ja')		// 日本語の投稿のみ
		.where('text', 'regexp', 'satisfactory|サティスファクトリー') 	// 正規表現検索
		.orderBy('indexedAt', 'desc')	// 日時でソート(降順)
		.orderBy('cid', 'desc')			// cidでソート(降順)
		.limit(params.limit)
